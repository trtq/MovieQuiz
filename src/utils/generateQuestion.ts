...
import { movieUrlRoot, MovieDB_API } from '_/utils/ApiAddresses';
...
export const generateQuestion = async (difficulty: DIFFICULTY): Promise<TQuestion | undefined> => {
  try {
    ...
    const moviesResp = await Axios(
      ...
      `${movieUrlRoot}/discover/movie?api_key=${MovieDB_API}&language=en-US&sort_by=vote_count.desc&page=${page}&primary_release_year=${year}&with_original_language=en`,
      { timeout: 3000 },
    );
    if (moviesResp.status !== 200 || !moviesResp.data) throw 'results are incorrect';
    if (moviesResp.data && moviesResp.data.results) {
      ...
      const similarResp = await Axios.get(
        `${movieUrlRoot}/movie/${movie.id}/similar?api_key=ed5a3609078d0ace7b6337f3145fc188&language=en-US&page=1`,
        { timeout: 3000 },
      );
      if (similarResp.status !== 200 || !similarResp.data) throw 'result for a similar movie search are incorrect';
